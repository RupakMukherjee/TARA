...
!mpi and accfft
type (c_ptr) :: fftPlan
integer mpierr, nprocs, myrank,allocmax,ngpus
type (c_ptr) :: comm
...
! Raynold's Number.
Re = ms*rho0*U0*Lx/mu; if (myrank.eq.0) write(5,*) "Raynold's Number =", Re
Rm = U0*Lx/eta; if (myrank.eq.0) write(5,*) "Magnetic Raynold's Number =", Rm
...
do k = 1, Nz/nprocs
  do j = 1, Ny
    do i = 1, Nx
      ...
      ux(i,j,k) = U0 * ( A*dsin(kf*z(k+((Nz/nprocs)*myrank))) + C*dcos(kf*y(j)) )
      uy(i,j,k) = U0 * ( B*dsin(kf*x(i)) + A*dcos(kf*z(k+((Nz/nprocs)*myrank))) )
...
write(*,*) "rank: ",myrank, ":Iteration", t, "t"
...
!    z(kglobal)=0.0d0+real(kglobal-1)*dz 
    ...
    ! Evaluate Forcing.
    Fx(i,j,k) = A*dsin(kf*z(kglobal)) + C*dcos(kf*y(j))
    Fy(i,j,k) = B*dsin(kf*x(i)) + A*dcos(kf*z(kglobal))
