...
extern "C" void createCUFFTPlan3D(void *plan, int nx, int ny, int nz, int planType, void *stream)
{
    cufftHandle *cPlan;

    cPlan = (cufftHandle *)plan;
    CHECK_CUFFT(cufftPlan3d(cPlan, nx, ny, nz, (cufftType)planType));
    CHECK_CUFFT(cufftSetStream(*cPlan,  (cudaStream_t)stream));
}

extern "C" void executeCUFFT3D(void *plan, void *iData, void *oData, int planType)
{
    cufftHandle *cPlan;
...
extern "C" void destroyCUFFTPlan3D(void *plan)
{
    cufftHandle *cPlan;
