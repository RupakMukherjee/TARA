...
for i in range (354100,525101,1000):
    ...
    ax.scatter(d1[0], d1[1], np.zeros(len(d1[0])), c = d1[3], lw = 0)
    ax.scatter(np.zeros(len(d2[0])), d2[0], d2[1], c = d2[3], lw = 0)
    p4 = ax.scatter(d3[1], np.zeros(len(d3[0])), d3[0], c = d3[3], lw = 0)
    plt.colorbar(p4)
    plt.savefig('Tracer_B_'+str(i)+'.png', format='png')
    #plt.show()
    #raw_input('')
    plt.close()
