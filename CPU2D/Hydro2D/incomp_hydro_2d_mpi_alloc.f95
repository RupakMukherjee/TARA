...
real (C_DOUBLE), dimension(:, :), allocatable :: psi,omega,ux,uy
real (C_DOUBLE), dimension(:, :), allocatable :: domega_dx,domega_dy
real (C_DOUBLE), dimension(:, :), allocatable :: ux_domega_dx,uy_domega_dy
complex (C_DOUBLE_COMPLEX), dimension(:, :), allocatable :: omegak,omegak_dum,omegak_new
complex (C_DOUBLE_COMPLEX), dimension(:, :), allocatable :: dt_omegak_old,dt_omegak_new
complex (C_DOUBLE_COMPLEX), dimension(:, :), allocatable :: psik,ukx,uky
complex (C_DOUBLE_COMPLEX), dimension(:, :), allocatable :: i_kx_omegak,i_ky_omegak
complex (C_DOUBLE_COMPLEX), dimension(:, :), allocatable :: NLkx,NLky,NLk
...
allocate(psi(Nx,Ny/nproc))
allocate(omega(Nx,Ny/nproc))
allocate(ux(Nx,Ny/nproc))
allocate(uy(Nx,Ny/nproc))
allocate(domega_dx(Nx,Ny/nproc))
allocate(domega_dy(Nx,Ny/nproc))
allocate(ux_domega_dx(Nx,Ny/nproc))
allocate(uy_domega_dy(Nx,Ny/nproc))

allocate(omegak(Nh,Ny/nproc))
allocate(omegak_dum(Nh,Ny/nproc))
allocate(omegak_new(Nh,Ny/nproc))
allocate(dt_omegak_old(Nh,Ny/nproc))
allocate(dt_omegak_new(Nh,Ny/nproc))
allocate(psik(Nh,Ny/nproc))
allocate(ukx(Nh,Ny/nproc))
allocate(uky(Nh,Ny/nproc))
allocate(i_kx_omegak(Nh,Ny/nproc))
allocate(i_ky_omegak(Nh,Ny/nproc))
allocate(NLkx(Nh,Ny/nproc))
allocate(NLky(Nh,Ny/nproc))
allocate(NLk(Nh,Ny/nproc))
...
do i = 1, Nx
  do j = 1, Ny/nproc
    ...
    omega(i,j) = W0/dcosh((y(j+((Ny/nproc)*myid))+0.50d0*pi)/d)**2.0-W0/dcosh((y(j+((Ny/nproc)*myid))-0.50d0*pi)/d)**2.0
    omega(i,j) = omega(i,j) - W0/dcosh((y(j+((Ny/nproc)*myid))+1.50d0*pi)/d)**2.0+W0/dcosh((y(j+((Ny/nproc)*myid))-1.50d0*pi)/d)**2.0
    omega(i,j) = omega(i,j)+0.01*dcos(m*x(i))
!    write(15+myid,*) x(i),y(j+((Ny/nproc)*myid)),omega(i,j),ux(i,j),uy(i,j)
  end do ! j
end do ! i
...
! Energy calculation and Data Printing.   
Energy = 0.0d0
...
!  do i = 1,Nx
!    do j = 1,local_Ny
!      write(t*10+110+myid,*) i, j+local_j_offset*local_Ny, omega(i,j)
!    end do ! j
!  end do ! i
!  close(t*10+110+myid)
!endif
