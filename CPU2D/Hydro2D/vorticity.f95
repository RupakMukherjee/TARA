...
integer ( kind = 4 ) i,j,t,m,n
real ( kind = 8 ) Lx,Ly,dx,dy,h,kx,ky,time,time_min,time_max,dt,nux,nuy,C0,C1,C2,C3,W0,d,G,Gb
...
!    if (t == 10000 .or. t == 0) then
    if (mod(dfloat(t),10.0) == 0.0) then
    write(t+105,*) x(i),y(j),ux(i,j),uy(i,j),omega(i,j)
    endif
  enddo
enddo  
...
! Tracer particles included...
  ...
  vxp(i) = ux(m,n) * (x(m+1)-xp(i))*(y(n+1)-yp(i)) * h + ux(m+1,n) * (xp(i)-x(m))*(y(n+1)-yp(i)) * h + &
           ux(m,n+1) * (x(m+1)-xp(i))*(yp(i)-y(n)) * h + ux(m+1,n+1) * (xp(i)-x(m))*(yp(i)-y(n)) * h
           
  vyp(i) = uy(m,n) * (x(m+1)-xp(i))*(y(n+1)-yp(i)) * h + uy(m+1,n) * (xp(i)-x(m))*(y(n+1)-yp(i)) * h + &
           uy(m,n+1) * (x(m+1)-xp(i))*(yp(i)-y(n)) * h + uy(m+1,n+1) * (xp(i)-x(m))*(yp(i)-y(n)) * h 

  endif
  
  if (m==Nx .and. n==Ny) then
  
  vxp(i) = ux(m,n) * (Lx-xp(i))*(Ly-yp(i)) * h + ux(1,n) * (xp(i)-x(m))*(Ly-yp(i)) * h + &
           ux(m,1) * (Lx-xp(i))*(yp(i)-y(n)) * h + ux(1,1) * (xp(i)-x(m))*(yp(i)-y(n)) * h
           
  vyp(i) = uy(m,n) * (Lx-xp(i))*(Ly-yp(i)) * h + uy(1,n) * (xp(i)-x(m))*(Ly-yp(i)) * h + &
           uy(m,1) * (Lx-xp(i))*(yp(i)-y(n)) * h + uy(1,1) * (xp(i)-x(m))*(yp(i)-y(n)) * h 
  
  endif
  
  if (m==Nx .and. n/=Ny) then
  
  vxp(i) = ux(m,n) * (Lx-xp(i))*(y(n+1)-yp(i)) * h + ux(1,n) * (xp(i)-x(m))*(y(n+1)-yp(i)) * h + &
           ux(m,n+1) * (Lx-xp(i))*(yp(i)-y(n)) * h + ux(1,n+1) * (xp(i)-x(m))*(yp(i)-y(n)) * h
           
  vyp(i) = uy(m,n) * (Lx-xp(i))*(y(n+1)-yp(i)) * h + uy(1,n) * (xp(i)-x(m))*(y(n+1)-yp(i)) * h + &
           uy(m,n+1) * (Lx-xp(i))*(yp(i)-y(n)) * h + uy(1,n+1) * (xp(i)-x(m))*(yp(i)-y(n)) * h 
  ...
  vxp(i) = ux(m,n) * (x(m+1)-xp(i))*(Ly-yp(i)) * h + ux(m+1,n) * (xp(i)-x(m))*(Ly-yp(i)) * h + &
           ux(m,1) * (x(m+1)-xp(i))*(yp(i)-y(n)) * h + ux(m+1,1) * (xp(i)-x(m))*(yp(i)-y(n)) * h
           
  vyp(i) = uy(m,n) * (x(m+1)-xp(i))*(Ly-yp(i)) * h + uy(m+1,n) * (xp(i)-x(m))*(Ly-yp(i)) * h + &
           uy(m,1) * (x(m+1)-xp(i))*(yp(i)-y(n)) * h + uy(m+1,1) * (xp(i)-x(m))*(yp(i)-y(n)) * h 
